module ApplicationHelper
  ...
  def upload_icon(file_mime_type)
    ...
    when 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
      tag.i class: 'fa-regular fa-2x fa-file-word'
    when 'application/vnd.ms-excel', 'application/xls', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
      tag.i class: 'fa-regular fa-2x fa-file-excel'
    when 'application/pdf'
      tag.i class: 'fa-regular fa-2x fa-file-pdf'
    when 'application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.presentation'
      tag.i class: 'fa-regular fa-2x fa-file-powerpoint'
    when 'text/csv'
      tag.i class: 'fa-regular fa-2x fa-file-csv'
    when 'application/octet-stream'
      tag.i class: 'fa-regular fa-2x fa-cube'
    else
      tag.i class: 'fa-regular fa-2x fa-file'
    end.html_safe
  end

  def list_errors_message(object, action, title)
    if object.errors.full_messages.any?
      "<div>Something went wrong #{action} the #{title}. Please review the problems below: <ul class=\"my--0 ml--2 pl--6\">#{object.errors.full_messages.uniq.collect do |error|
        "<li>#{error}</li>"
      end.join}</ul></div>".html_safe
    else
